...
//
#define add_overflow_32 (rn_contents > INT_MAX - op2)   // `rn + op2` will overflow (signed)
#define sub_overflow_32 (rn_contents < INT_MIN + op2)   // `rn - op2` will underflow (signed)
#define uadd_overflow_32 (rn_contents > UINT_MAX - op2) // `rn + op2` will produce a carry (unsigned)
#define usub_overflow_32 (rn_contents < op2)            // `rn - op2` will produce a borrow (unsigned)

// we need to adapt the MAX/MIN values for 64-bit
#define add_overflow_64 (rn_contents > LONG_MAX - op2)   // `rn + op2` will overflow (signed)
#define sub_overflow_64 (rn_contents < LONG_MIN + op2)   // `rn - op2` will underflow (signed)
...
void perform_data_processing_immediate(
	...
	if (data.opi == 2) {
		union arithmetic_immediate_operand operand = (union arithmetic_immediate_operand) {.bits = instr.data};
		if (instr.sf == 1) {
		...
		union wide_move_operand operand = (union wide_move_operand)  {.bits = instr.data};
		if (instr.sf == 1) {
...
uint64_t arithmetic_helper_64(
	CPU *cpu,
	unsigned opc,
	uint64_t rn_contents,
	uint64_t op2
) {
	uint64_t result;
	switch (opc) {
...
uint32_t arithmetic_helper_32(
	CPU *cpu,
	unsigned opc,
	uint32_t rn_contents,
	uint32_t op2
) {
	uint32_t result;
	switch (opc) {
...
void perform_data_processing_register (
	...
	if (instr.maybe_M == 1) {
		union multiply_operand operand = (union multiply_operand) {.bits = data.operand};
		...
		union arithmetic_logic_opr opr = (union arithmetic_logic_opr) {.bits = data.opr};
		if (opr.type  == 1) {
			if (instr.sf == 1) {
...
void multiply_64(
	...
	uint64_t result;
	int64_t rnrm = (int64_t) (read_register64(cpu, data.rn) * read_register64(cpu, data.rm));
	if (operand.x == 1) {
		rnrm = -rnrm;
	}
...
void multiply_32(
	...
	int32_t rnrm = (int32_t) (read_register32(cpu, data.rn) * read_register32(cpu, data.rm));
	if (operand.x == 1) {
		rnrm = -rnrm;
	}
...
void data_processing_init(CPU *cpu, uint32_t instruction, bool is_immediate) {
	union data_processing_instruction instr = (union data_processing_instruction) {.bits = instruction};
	if (is_immediate) {

...
#define uadd_overflow_32 \
  (op2 > 0 &&            \
   rn_contents >         \
       (uint32_t)(UINT_MAX - op2))  // `rn_contents + op2` would overflow (1)
...
#define add_underflow_64    \
  (((int64_t)op2) < 0 &&    \
   ((int64_t)rn_contents) < \
       LONG_MIN - ((int64_t)op2))  // `rn_contents + op2` would underflow (2)
...
#define sub_underflow_64    \
  (((int64_t)op2) > 0 &&    \
   ((int64_t)rn_contents) < \
       LONG_MIN + ((int64_t)op2))  // `rn_contents - op2` would underflow (4)
// unsigned
#define uadd_overflow_64 \
  (op2 > 0 &&            \
   rn_contents >         \
       (uint64_t)(ULONG_MAX - op2))  // `rn_contents + op2` would overflow (1)
...
void arithmetic_immediate_64(CPU *cpu, union data_processing_instruction instr,
  ...
  assert(
      instr.rd != stack_pointer_encoding ||
      !(instr.opc == 0b00 || instr.opc == 0b10));  // not handling stack pointer
...
void multiply_64(CPU *cpu, union data_processing_instruction instr,
  ...
  uint64_t result;
  int64_t rnrm =
      (int64_t)(read_register64(cpu, data.rn) * read_register64(cpu, data.rm));
  if (operand.x == 1) {
    rnrm = -rnrm;
  }
...
void multiply_32(CPU *cpu, union data_processing_instruction instr,
                 ...
                 union multiply_operand operand) {
  int32_t rnrm =
      (int32_t)(read_register32(cpu, data.rn) * read_register32(cpu, data.rm));
  if (operand.x == 1) {
    rnrm = -rnrm;
  }

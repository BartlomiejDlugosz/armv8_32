...
dynamicString createNewDynamicString(int starting_number_of_elements) {
  ...
  new_dynamic_string->data = malloc(starting_number_of_elements * sizeof(char));
  if (new_dynamic_string->data == NULL) {
    fprintf(stderr, "An error occured allocating memory");
    exit(1);
  }
  memset(new_dynamic_string->data, 0, starting_number_of_elements * sizeof(char));
...
char getChar(dynamicString arr, int index) {
  return arr->data[index];
}

char *getString(dynamicString arr) {
  return arr->data;
}

...
#define OPC_CASE_ERROR()                                                                         \
    fprintf(stderr, "Invalid operation code (opc) encountered during instruction execution.\n"); \
    exit(EXIT_FAILURE);

#define SHIFT_TYPE_DETERMINATION_ERROR()                                                                              \
    fprintf(stderr, "Invalid or unsupported shift type encountered for operand op2 during instruction execution.\n"); \
    exit(EXIT_FAILURE);

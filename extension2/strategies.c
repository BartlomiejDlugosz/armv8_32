...
static bool cars_waiting(intersection *isec) {
    ...
    for (int i = 0; i < NUM_ROADS; i++) {
        cars_waiting_bool |= (isec->roads[i]->light->clr == RED &&
                              isec->roads[i]->head_car != NULL);
    }
...
static double sensor_significance(intersection *isec) {
    if (isec->roads[0]->light->clr == RED &&
        isec->roads[0]->light->has_sensor) {
...
static bool amber_lights(intersection *isec) {
    ...
    for (int i = 0; i < NUM_ROADS; i++) {
        amber_light |= (isec->roads[i]->light->clr == RED_AMBER ||
                        isec->roads[i]->light->clr == AMBER);
    }
    return amber_light;
}
...
bool basic_plus(intersection *isec, time_t time_since_change,
    ...
    if (amber_lights(isec) && time_since_change > TIME_TO_CHANGE_AMBER) {
        return true;
    } else if (time_since_change >
               TIME_TO_CHANGE_BASIC * sensor_significance(isec)) {

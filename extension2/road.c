...
void update_distances(road *update_road, time_t dt) {
    ...
    if (update_road->light->clr == RED) {
        ...
        while (next_car != NULL && distance_covered > 0) {
            // Previous Distance
            int distance_to_car_in_front_prev =
                next_car->distance_to_car_in_front;
            // Either the reaches the maximum follow distance allowed or covers
            // some distance
            next_car->distance_to_car_in_front =
                MAX(follow_distance,
                    next_car->distance_to_car_in_front - distance_covered);
            // Distance covered is decreased by the distance covered by the car
            distance_covered -= distance_to_car_in_front_prev -
                                next_car->distance_to_car_in_front;
            // If the cars in front have not moved and the distance covered by
            // the car is 0, it has remained stationary
            if (next_car->distance_to_car_in_front -
                        distance_to_car_in_front_prev ==
                    0 &&
                all_cars_in_front_are_stationary) {
...
// pop off ANY cars which have passed stop line (checking for negative
// distances), that involves potentially popping many cars, updating the running
// head and its distance to the light
car *remove_crossed(road *cur_road) {
    ...
    // Keep track of last popped inorder to complete the linked list of cars
    // that have crossed
    car *last_popped = NULL;
    int distance_travelled = cur_road->head_car->distance_to_car_in_front;
    // If the distance travelled is greater than zero, this means that the car
    // has to still reach the light Remember for the HEAD car, the distance to
    // car in front represents the distance to the light If distance travelled
    // is zero, the car is at the traffic light and has not yet crossed
    if (distance_travelled >= 0) {
        return NULL;
    }
    // Dealing with the rest of the cars
    while (distance_travelled < 0 && cur_road->head_car != NULL) {
        // Check if there is a next car, in which case we need to update
        // distance travelled
        if (cur_road->head_car->next != NULL) {
            distance_travelled +=
                cur_road->head_car->next->distance_to_car_in_front;
        }
        // Update last popped
...
void free_single_car(car *current_car) { free(current_car); }
